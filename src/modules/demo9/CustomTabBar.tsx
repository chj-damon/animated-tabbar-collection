  ...
  return (
    <View style={[styles.tabBar, { height: 60 + insets.bottom, paddingBottom: insets.bottom }]}>
      {routes.map((route, index) => {
        ...
        const icon = tabBarIcon?.({ focused, color: focused ? '#0189fb' : '#333', size: 24 }) as any as ReactElement;

        return (
          <TabItem key={index} style={styles.item} icon={icon} active={focused} onPress={() => onTabPress(index)} />
        );
      })}
    </View>
  );
